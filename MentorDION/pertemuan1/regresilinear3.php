...
<body>
    <form action="regresilinear3.php" method="post">
        <?php 
...
if($_POST['submit']??''){
    ...
    for ($m=0; $m <$jumlahsempel ; $m++) { 

        $XY[$m] = $Xhari[$m]*$Ypenjualan[$m];
        $Xkuadrat[$m]= $Xhari[$m]*2;
        $sumXkuadrat=array_sum($Xkuadrat);
        # code...
        $sumXY=array_sum($XY);
        $sumY=array_Sum($Ypenjualan);
        $sumX=array_Sum($Xhari);

        $Bpembilang=$jumlahsempel*($sumXY)- ($sumX*$sumY);
        $Bpenyebut=($jumlahsempel*$sumXkuadrat)-($sumX*2);
        $B=$Bpembilang/$Bpenyebut;

        $A=$sumY-($B*$sumX)/$jumlahsempel;

        $Ypre=$A-$B*$hariprediksi;
        $AnalisisErro[$m]=$Ypre-$Ypenjualan[$m];
    }
